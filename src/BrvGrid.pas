...
type
  ...
  TBrvGridOption = (brvgoMultiCellSelect, brvgoThemed, brvgoExcel);
  TBrvGridOptions = set of TBrvGridOption;
  TBrvGridInplaceEditorType = (brvgetEdit, brvgetEditInteger, brvgetEditFloat, brvgetCombo, brvgetComboNoEditText);

  TBrvGrid = class;

  TOnFormatText = procedure (Grid: TBrvGrid; X,Y: Integer; var CellText: string) of object;

  TBrvColumn = class(TCollectionItem)
  private
    FTitle: string;
    FFooter: string;
    FWidth: Integer;
    ...
    FHint: string;
    FReadOnly: Boolean;
    FEditorType: TBrvGridInplaceEditorType;
    FOnFormatText: TOnFormatText;
    function GetGrid: TBrvGrid;
  ...
  published
    property Grid: TBrvGrid read GetGrid;
    ...
    property EditorType: TBrvGridInplaceEditorType read FEditorType write FEditorType;
    property OnFormatText: TOnFormatText read FOnFormatText write FOnFormatText;
  end;


  TBrvColumns = class(TCollection)
  private
    FGrid: TBrvGrid;
    function GetItem(Index: Integer): TBrvColumn;
    procedure SetItem(Index: Integer; Value: TBrvColumn);
  protected
    function GetOwner: TPersistent; override;
    procedure Update(Item: TCollectionItem); override;
  public
    constructor Create(AGrid: TBrvGrid);
    property Grid: TBrvGrid read FGrid;
    property Items[Index: Integer]: TBrvColumn read GetItem write SetItem; default;
    function Add: TBrvColumn;
    function AddItem(Item: TBrvColumn; Index: Integer): TBrvColumn;
    function Insert(Index: Integer): TBrvColumn;
  end;


  IBrvGridInplaceEditor = interface
  ...
  TBrvInplaceEdit = class(TEdit, IBrvGridInplaceEditor)
  private
    m_Mediator: TBrvInplace;
    m_Alignment: THorzAlign;
    m_nCellX, m_nCellY: Integer;
    m_EditorType: TBrvGridInplaceEditorType;
  ...
  public
    constructor Create(AMediator: TBrvInplace); reintroduce;
    procedure ShowEdit(X, Y: Integer);
    procedure HideEdit;
  end;

  TBrvInplaceCombo = class(TComboBox, IBrvGridInplaceEditor)
  private
    m_Mediator: TBrvInplace;
  ...
  public
    ...
    procedure ShowEdit(X, Y: Integer);
    procedure HideEdit;
    property Mediator: TBrvInplace read m_Mediator;
  end;
  ...
  private
    ...
    procedure Clear;
  public
    constructor Create(AGrid: TBrvGrid); reintroduce;
    procedure ShowEdit(X, Y: Integer);
    procedure HideEdit;
    property Grid: TBrvGrid read m_Grid;
  ...
  TOnEditorCreated = procedure (Grid: TBrvGrid; EditorControl: TWinControl) of object;

  TOnGetCellColor = function (Grid: TBrvGrid; Col, Row: Integer; var Handled: boolean): TColor of object;

  TBrvGridRowsByIndexEnumerator = record
    m_NiceGrid: TBrvGrid;
    m_nRowIndex: Integer;
    constructor Create(ANiceGrid: TBrvGrid);
    function GetEnumerator: TBrvGridRowsByIndexEnumerator;
    function MoveNext(): Boolean;
    function GetCurrent(): Integer;
    property Current: Integer read GetCurrent;
  end;

  TBrvGrid = class(TCustomPanel)
  private
    ...
    procedure SetColumns(Value: TBrvColumns);
    ...
    function CreateColumn: TBrvColumn;
    ...
    procedure SetOptions(const Value: TBrvGridOptions);
  ...
  public
    ...
    procedure TryEdit;
    procedure EndEdit;
    function RowsByIndex: TBrvGridRowsByIndexEnumerator;
  ...
  published
    ...
    property Columns: TBrvColumns read FColumns write SetColumns;
    ...
    property Options: TBrvGridOptions read FOptions write SetOptions;
    property Font;
    property Anchors;
    property Align;
    property BevelKind;
...
constructor TBrvGrid.Create(AOwner: TComponent);
begin
  ...
  FColumns := TBrvColumns.Create(Self);
  FEdit := TBrvInplace.Create(Self);
  FGutterStrings := TStringList.Create;
  Mergeds := TList.Create;

  FOptions := [brvgoThemed];
...
procedure TBrvGrid.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.Style := Params.Style or WS_HSCROLL or WS_VSCROLL;
end;

procedure TBrvGrid.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TBrvGrid.SetScrollBar(AKind, AMax, APos, AMask: Integer);
...
procedure TBrvGrid.ShowHideScrollBar(HorzVisible, VertVisible: Boolean);
begin
  ShowScrollBar(Handle, SB_HORZ, HorzVisible);
  ShowScrollBar(Handle, SB_VERT, VertVisible);
end;

procedure TBrvGrid.WMHScroll(var Msg: TWMVScroll);
var
  Old: Integer;
begin
  //ForceHideCaret;
...
procedure TBrvGrid.WMVScroll(var Msg: TWMHScroll);
var
  Old: Integer;
begin
  //ForceHideCaret;
...
procedure TBrvGrid.SetColCount(Value: Integer);
begin
...
procedure TBrvGrid.SetRowCount(Value: Integer);
begin
...
procedure TBrvGrid.ClearHeaderInfos;
var
  x: Integer;
  P: PHeaderInfo;
begin
...
procedure TBrvGrid.Recalculate;
var
  x: Integer;
...
function TBrvGrid.GetCellColor(X, Y: Integer): TColor;
{-----------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    begin
      if not ((X = FCol) and (y = FRow)) and (FSelectionColor<>clNone) and not (brvgoThemed in Options)
...
procedure TBrvGrid.DrawFixCell(Rc: TRect; Str: string; AFont: TFont; AEvent: TOnDrawHeaderEvent);
var
  Rt: TRect;
  Handled: Boolean;
begin
...
procedure TBrvGrid.RenderGutter;
const
  ArrowWidth = 8;
var
  x: Integer;
...
procedure TBrvGrid.RenderHeader;
var
  x: Integer;
  R, Dummy: TRect;
  P: PHeaderInfo;
...
function TBrvGrid.RowsByIndex: TBrvGridRowsByIndexEnumerator;
{-----------------------------------------------------------------------------
  Procedure: RowsByIndex
  Author:    nbi
  Date:      16-Oct-2014
  Arguments: None
  Result:    TBrvGridRowsByIndexEnumerator
-----------------------------------------------------------------------------}
begin
  Result := TBrvGridRowsByIndexEnumerator.Create(self);
end;




procedure TBrvGrid.RenderFooter;
var
  x: Integer;
  R, Dummy: TRect;
...
procedure TBrvGrid.DrawBackground(X, Y: Integer);
{-----------------------------------------------------------------------------
...
procedure TBrvGrid.DrawText(X, Y: Integer);
{-----------------------------------------------------------------------------
  Procedure: DrawText
  Author:    nbi
  Date:      16-Oct-2014
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(FOnFormatText)
        then FOnFormatText(Self, X, Y, CellText);

      if Assigned(Columns[X].FOnFormatText)
        then Columns[X].FOnFormatText(Self, X, Y, CellText);
...
function TBrvGrid.FastDrawText(X, Y: Integer; IsEditing: Boolean): TPoint;
var
  R, Dummy: TRect;
  Handled: Boolean;
  Column: TBrvColumn;
...
procedure TBrvGrid.DrawSelection;
var
  R, R1, R2: TRect;
...
procedure TBrvGrid.ClearUnused;
var
  t: Integer;
begin
...
procedure TBrvGrid.UpdateHeader;
var
  P: PHeaderInfo;
  x, y: Integer;
  t: TStringList;
...
function TBrvGrid.GetColCoord(I: Integer): Integer;
var
  x: Integer;
  Column: TBrvColumn;
begin
...
function TBrvGrid.GetCellRect(x, y: Integer): TRect;
var
  l, t, w, h: Integer;
begin
  if (x = -1) or (y = -1) then
...
function TBrvGrid.CellRectToClient(R: TRect): TRect;
begin
  Result := R;
  OffsetRect(Result, - FHorzOffset + FixedWidth, - FVertOffset + FixedHeight);
end;

function TBrvGrid.GetCellAtPos(X, Y: Integer): TPoint;
var
  ax, ay: Integer;
begin
...
function TBrvGrid.GetColFromX(X: Integer): Integer;
var
  ax: Integer;
begin
...
function TBrvGrid.GetRowFromY(Y: Integer): Integer;
var
  ay: Integer;
begin
...
function TBrvGrid.SafeGetCell(X, Y: Integer): string;
var
  t: TStringList;
begin
  Result := '';
...
function TBrvGrid.GetCell(X, Y: Integer): string;
var
  t: TStrings;
begin
  Result := '';
...
procedure TBrvGrid.InternalSetCell(X, Y: Integer; Value: string;
...
begin
  ...
  begin
    ...
    t[Y] := Value;
  if not FUpdating
    then FastDrawText(X, Y, False);
end;

procedure TBrvGrid.SetCell(X, Y: Integer; Value: string);
begin
  InternalSetCell(X, Y, Value, False);
end;

procedure TBrvGrid.BeginUpdate;
begin
  FUpdating := True;
  //ForceHideCaret;
end;
...
procedure TBrvGrid.SetFlat(Value: Boolean);
begin
  if (FFlat <> Value) then
  begin
    FFlat := Value;
...
procedure TBrvGrid.SetColor(Value: TColor);
begin
...
procedure TBrvGrid.SetAlternateColor(Value: TColor);
begin
...
procedure TBrvGrid.SetGridColor(Value: TColor);
begin
...
function TBrvGrid.GetColWidths(Index: Integer): Integer;
begin
  Result := FColumns[Index].FWidth;
end;

procedure TBrvGrid.SetColWidths(Index, Value: Integer);
begin
...
procedure TBrvGrid.SetAutoColWidth(Value: Boolean);
begin
...
procedure TBrvGrid.SetDefColWidth(Value: Integer);
begin
...
procedure TBrvGrid.SetDefRowHeight(Value: Integer);
begin
...
procedure TBrvGrid.SetFitToWidth(Value: Boolean);
begin
...
procedure TBrvGrid.SetHeaderColor(Value: TColor);
begin
...
procedure TBrvGrid.SetHeaderDarkColor(Value: TColor);
begin
...
procedure TBrvGrid.SetHeaderLightColor(Value: TColor);
begin
...
procedure TBrvGrid.SetHeaderLine(Value: Integer);
begin
...
procedure TBrvGrid.SetSelectionColor(Value: TColor);
begin
...
procedure TBrvGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
  l, t, r, b: Integer;
  x, y: Integer;
  Empty: Boolean;
...
begin
  ...
  begin
    ...
    case Key of
      ...
      VK_RETURN:
        begin
          if not (brvgoExcel in Options) then begin
            TryEdit;
          end else begin
            //
            Str := GetCell(Col, Row);
...
procedure TBrvGrid.KeyPress(var Key: Char);
{-----------------------------------------------------------------------------
  Procedure: KeyPress
  Author:    nbi
  Date:      16-Oct-2014
  Arguments: var Key: Char
  Result:    None
-----------------------------------------------------------------------------}
var
  //Pt: TPoint;
...
function TBrvGrid.GetHitTestInfo(X, Y: Integer): TGridHitTest;
var
  a, i1, i2: Integer;
  ax, ay: Integer;
  IsSizing: Boolean;
...
begin
  ...
  if (brvgoExcel in Options) and PtInRect(SmallBox, Point(X, Y))
...
procedure TBrvGrid.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
var
  Pt: TPoint;
...
procedure TBrvGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  Total2Col: Integer;
  Suggested: Integer;
  Pt: TPoint;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if(brvgoMultiCellSelect in Options) then
          SetSelectArea(Rect(l, t, r, b))
        else
...
procedure TBrvGrid.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
var
  Ls: TList;
  ax, ay: Integer;
  ...
  begin
    ...
    ix := ((ix - l) mod w) + l;
    iy := ((iy - t) mod h) + t;
    Result := SafeGetCell(TBrvColumn(Ls[ix]).Index, iy);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (TBrvColumn(Ls[ax]).Index = SmallBoxArea.Left) then
        begin
          l := ax;
          Break;
        end;
      ...
      for ax := 0 to Ls.Count-1 do
        for ay := FSelectArea.Top to FSelectArea.Bottom
          do InternalSetCell(TBrvColumn(Ls[ax]).Index, ay, GetCopy(ax, ay), True);
      Ls.Free;
      SmallBoxArea := Rect(-1, -1, -1, -1);
      BuffString := '';
      FUpdating := False;
...
procedure TBrvGrid.SetColumns(Value: TBrvColumns);
begin
  FColumns.Assign(Value);
end;

function TBrvGrid.CreateColumn: TBrvColumn;
begin
  Result := TBrvColumn.Create(Columns);
end;

procedure TBrvGrid.UpdateColumn(Index: Integer);
var
  l, w: Integer;
  Rc: TRect;
begin
...
procedure TBrvGrid.UpdateColumns;
begin
  UpdateHeader;
  Invalidate;
end;

function TBrvGrid.GetColCount: Integer;
begin
  Result := FColumns.Count;
end;

function TBrvGrid.TotalWidth: Integer;
var
  x: Integer;
begin
  Result := 0;
...
procedure TBrvGrid.CMFontChanged(var Msg: TMessage);
var
  x: Integer;
begin
  inherited;
...
procedure TBrvGrid.WMSize(var Msg: TMessage);
begin
  inherited;
  Recalculate;
  if (FColumns.Count > 0)
...
procedure TBrvGrid.WMEraseBkgnd(var Msg: TWMEraseBkGnd);
begin
  Msg.Result := 1;
end;

procedure TBrvGrid.CMWantSpecialKey(var Message: TWMKey);
begin
  inherited;
  with Message do
  case CharCode of
...
procedure TBrvGrid.SetShowGrid(Value: Boolean);
begin
...
procedure TBrvGrid.SetShowFooter(const Value: Boolean);
begin
...
procedure TBrvGrid.SetHorzOffset(Value: Integer);
begin
...
procedure TBrvGrid.SetVertOffset(Value: Integer);
begin
...
procedure TBrvGrid.EnsureVisible(X, Y: Integer);
var
  t, b, h: Integer;
  l, r: Integer;
  Horz, Vert: Boolean;
...
function TBrvGrid.HeaderCells(I: Integer): THeaderInfo;
begin
  Result := PHeaderInfo(FHeaderInfos[I])^;
end;

function TBrvGrid.HeaderCellsCount: Integer;
begin
  Result := FHeaderInfos.Count;
end;

procedure TBrvGrid.SetReadOnly(Value: Boolean);
begin
...
procedure TBrvGrid.SetCol(Value: Integer);
begin
  if (FCol <> Value) then
  begin
    //ForceHideCaret;
...
procedure TBrvGrid.SetRow(Value: Integer);
begin
  if (FRow <> Value) then
  begin
    //ForceHideCaret;
...
procedure TBrvGrid.AdjustSelection(Value: TRect; Force: Boolean);
var
  Old, Rc: TRect;
  R1, R2, R: TRect;
begin
...
procedure TBrvGrid.SetSelectArea(Value: TRect);
begin
...
//
//procedure TBrvGrid.ForceHideCaret;
//begin
//  if CaretVisible
...
//end;
//
//procedure TBrvGrid.ForceShowCaret;
//begin
//  if not CaretVisible
...
procedure TBrvGrid.WMKillFocus(var Msg: TWMKillFocus);
{-----------------------------------------------------------------------------
...
procedure TBrvGrid.WMSetFocus(var Msg: TWMSetFocus);
{-----------------------------------------------------------------------------
...
procedure TBrvGrid.SetGutterKind(Value: TGutterKind);
var
  Rc: TRect;
  RedrawAll: Boolean;
  Old: TGutterKind;
...
procedure TBrvGrid.SetGutterWidth(Value: Integer);
begin
...
procedure TBrvGrid.CopyToClipboard;
var
  s: string;
  t: TStringList;
  x, y: Integer;
...
procedure TBrvGrid.CutToClipboard;
var
  s: string;
  t: TStringList;
  x, y: Integer;
...
procedure TBrvGrid.PasteFromClipboard;
var
  tr, tc: TStringList;
  x, y: Integer;
  s: string;
...
procedure TBrvGrid.InvalidateCells;
var
  Rc: TRect;
begin
...
procedure TBrvGrid.InvalidateDownWard(Top: Integer);
var
  Rc: TRect;
begin
...
procedure TBrvGrid.InvalidateRightWard(Left: Integer);
var
  Rc: TRect;
begin
...
procedure TBrvGrid.NormalizeVertOffset;
begin
  FVertOffset := (FVertOffset div FDefRowHeight) * FDefRowHeight;
end;

procedure TBrvGrid.SetGutterFont(Value: TFont);
begin
  FGutterFont.Assign(Value);
  InvalidateGutter;
end;

procedure TBrvGrid.SetHeaderFont(Value: TFont);
begin
  FHeaderFont.Assign(Value);
  InvalidateHeader;
end;

procedure TBrvGrid.SetFooterFont(const Value: TFont);
begin
  FFooterFont.Assign(Value);
  Invalidate;
end;

procedure TBrvGrid.InvalidateGutter;
var
  Rc: TRect;
begin
  Rc := Rect(0, FixedHeight, FixedWidth, ClientHeight);
...
procedure TBrvGrid.InvalidateHeader;
var
  Rc: TRect;
begin
...
procedure TBrvGrid.HeaderFontChange(Sender: TObject);
begin
  InvalidateHeader;
end;

procedure TBrvGrid.GutterFontChange(Sender: TObject);
begin
  InvalidateGutter;
end;

procedure TBrvGrid.FooterFontChange(Sender: TObject);
begin
  Invalidate;
end;

function TBrvGrid.GetFirstVisible: Integer;
var
  x: Integer;
begin
  Result := -1;
...
function TBrvGrid.GetLastVisible: Integer;
var
  x: Integer;
begin
  Result := -1;
...
function TBrvGrid.GetNextVisible(Index: Integer): Integer;
var
  x: Integer;
begin
  Result := Index;
...
function TBrvGrid.GetPrevVisible(Index: Integer): Integer;
var
  x: Integer;
begin
  Result := Index;
...
procedure TBrvGrid.DeleteRow(ARow: Integer);
var
  x, y: Integer;
begin
  //ForceHideCaret;
...
procedure TBrvGrid.InsertRow(ARow: Integer);
var
  x: Integer;
begin
  //ForceHideCaret;
...
function TBrvGrid.AddRow: Integer;
var
  x: Integer;
  n: Integer;
begin
...
procedure TBrvGrid.WMUnknown(var Msg: TMessage);
begin
  Msg.Result := 0;
end;

procedure TBrvGrid.WMMouseWheel(var Msg: TWMMouseWheel);
var
  Old: Integer;
begin
  Old := FVertOffset;
...
procedure TBrvGrid.ColRowChanged;
begin
//  if Assigned(Sync)
//    then Sync.Row := FRow;
...
procedure TBrvGrid.Notification(AComponent: TComponent;
...
procedure TBrvGrid.SetGutterStrings(const Value: TStrings);
begin
...
function TBrvGrid.GetObject(X, Y: Integer): TObject;
var
  t: TStrings;
begin
  Result := nil;
...
procedure TBrvGrid.SetObject(X, Y: Integer; const Value: TObject);
var
  t: TStrings;
begin
  if (X > ColCount-1) or (Y > FRowCount-1)
...
function TBrvGrid.IsEmptyRow(y: Integer): Boolean;
{-----------------------------------------------------------------------------
...
function TBrvGrid.IsThemed: Boolean;
{-----------------------------------------------------------------------------
  Procedure: IsThemed
  Author:    nbi
  Date:      15-Oct-2014
...
-----------------------------------------------------------------------------}
begin
  Result := (brvgoThemed in Options) and GetWin7Themed;
end;



procedure TBrvGrid.SetOptions(const Value: TBrvGridOptions);
{-----------------------------------------------------------------------------
  Procedure: SetOptions
  Author:    nbi
  Date:      15-Oct-2014
  Arguments: const Value: TBrvGridOptions
  Result:    None
-----------------------------------------------------------------------------}
begin
  if(FOptions * [brvgoMultiCellSelect]) <> (value * [brvgoMultiCellSelect]) then begin

    if not (brvgoMultiCellSelect in Value) then
      with FSelectArea do
        SetSelectArea(rect(Left, top, Left, top));

  end;

  if(FOptions * [brvgoThemed]) <> (value * [brvgoThemed]) then begin
...
procedure TBrvGrid.ClearMergeCells;
var
  x, y: Integer;
  List: TStrings;
begin
...
function TBrvGrid.MergeCells(const X1, Y1, X2, Y2: Integer;
...
procedure TBrvGrid.BuildMergeData;
var
  Rc: TRect;
  x, y, z: Integer;
begin
...
procedure TBrvGrid.DrawMergedCell(Index: Integer);
var
  Data: TMergeCell;
  R, Rc, Dummy: TRect;
  l1, l2, t, h: Integer;
...
function TBrvGrid.GetHeaderInfo: TList;
begin
  Result := FHeaderInfos;
end;

function TBrvGrid.GetMergedCellsData: TList;
begin
  Result := Mergeds;
end;

procedure TBrvGrid.SetEnabled(const Value: Boolean);
begin
...
constructor TBrvColumn.Create(Collection: TCollection);
begin
  ...
  FTag := 0;
  FTag2 := 0;
  with TBrvColumns(Collection).Grid do
  begin
...
procedure TBrvColumn.Assign(Source: TPersistent);
begin
  if (Source is TBrvColumn) then
  begin
    Title     := TBrvColumn(Source).Title;
    Footer    := TBrvColumn(Source).Footer;
    Width     := TBrvColumn(Source).Width;
    Font      := TBrvColumn(Source).Font;
    Color     := TBrvColumn(Source).Color;
    HorzAlign := TBrvColumn(Source).HorzAlign;
    VertAlign := TBrvColumn(Source).VertAlign;
    Visible   := TBrvColumn(Source).Visible;
    Tag       := TBrvColumn(Source).Tag;
    Tag2      := TBrvColumn(Source).Tag2;
    Hint      := TBrvColumn(Source).Hint;
    CanResize := TBrvColumn(Source).CanResize;
    ReadOnly  := TBrvColumn(Source).ReadOnly;
    Strings.Assign(TBrvColumn(Source).Strings);
    Changed(False);
  end;
end;

procedure TBrvColumn.SetColor(Value: TColor);
begin
  if (FColor <> Value) then
  begin
    FColor := Value;
...
procedure TBrvColumn.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
  Changed(False);
end;

procedure TBrvColumn.SetHorzAlign(Value: THorzAlign);
begin
...
procedure TBrvColumn.SetTitle(Value: string);
begin
...
procedure TBrvColumn.SetFooter(const Value: string);
begin
...
procedure TBrvColumn.SetVertAlign(Value: TVertAlign);
begin
...
procedure TBrvColumn.SetWidth(Value: Integer);
begin
...
procedure TBrvColumn.SetVisible(Value: Boolean);
begin
  if (FVisible <> Value) then
  begin
    FVisible := Value;
    TBrvColumns(Collection).FGrid.ForcedColumn := Index;
    Changed(True);
  end;
end;

procedure TBrvColumn.SetStrings(Value: TStrings);
begin
  FStrings.Assign(Value);
  Changed(False);
end;

procedure TBrvColumn.FontChange(Sender: TObject);
begin
  Changed(False);
end;

function TBrvColumn.IsFontStored: Boolean;
begin
  Result := True;
  with TBrvColumns(Collection).FGrid.Font do
  begin
...
function TBrvColumn.GetGrid: TBrvGrid;
begin
  Result := TBrvColumns(Collection).FGrid;
end;

function TBrvColumn.GetDisplayName: string;
begin
  if (FTitle <> '')
    then Result := FTitle
...
constructor TBrvColumns.Create(AGrid: TBrvGrid);
begin
  inherited Create(TBrvColumn);
  FGrid := AGrid;
end;

function TBrvColumns.Add: TBrvColumn;
begin
  Result := TBrvColumn(inherited Add);
end;

function TBrvColumns.GetItem(Index: Integer): TBrvColumn;
begin
  Result := TBrvColumn(inherited GetItem(Index));
end;

procedure TBrvColumns.SetItem(Index: Integer; Value: TBrvColumn);
begin
  inherited SetItem(Index, Value);
end;

function TBrvColumns.GetOwner: TPersistent;
begin
  Result := FGrid;
end;

function TBrvColumns.Insert(Index: Integer): TBrvColumn;
begin
  Result := AddItem(nil, Index);
end;

function TBrvColumns.AddItem(Item: TBrvColumn;
  Index: Integer): TBrvColumn;
begin
  if (Item = nil)
...
procedure TBrvColumns.Update(Item: TCollectionItem);
begin
  if (Item <> nil)
...
procedure TBrvInplaceEdit.CMWantSpecialKey(var Message: TWMKey);
{-----------------------------------------------------------------------------
...
constructor TBrvInplaceEdit.Create(AMediator: TBrvInplace);
{-----------------------------------------------------------------------------
  Procedure: Create
  Author:    nbi
  Date:      15-Oct-2014
  Arguments: AMediator: TBrvInplace
  Result:    None
-----------------------------------------------------------------------------}
begin
  m_Mediator := AMediator;
...
procedure TBrvInplaceEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TBrvInplaceEdit.SetAlignment(Value: THorzAlign);
begin
...
procedure TBrvInplaceEdit.ShowEdit(X, Y: Integer);
{-----------------------------------------------------------------------------
  Procedure: ShowEdit
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplaceEdit.WMKillFocus(var Msg: TWMKillFocus);
{-----------------------------------------------------------------------------
  Procedure: WMKillFocus
  Author:    nbi
  Date:      15-Oct-2014
...
//procedure TBrvInplaceEdit.WndProc(var Message: TMessage);
//begin
...
procedure TBrvInplaceEdit.HideEdit;
{-----------------------------------------------------------------------------
  Procedure: HideEdit
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplaceEdit.Change;
begin
  inherited;
...
procedure TBrvInplaceEdit.KeyDown(var Key: Word; Shift: TShiftState);
{-----------------------------------------------------------------------------
  Procedure: KeyDown
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplaceEdit.KeyPress(var Key: Char);
{-----------------------------------------------------------------------------
  Procedure: KeyPress
  Author:    nbi
  Date:      16-Oct-2014
...
var
  ...
  begin
    if(m_EditorType=brvgetEdit) then
      EXIT;

    if(m_EditorType=brvgetEditFloat) and (key=FormatSettings.DecimalSeparator) then begin
      /// do not allow second decimal separator in text
      if(pos(key, text)>0) then
        Allowed := False;
    end
    else
    if(m_EditorType in [brvgetEditInteger, brvgetEditFloat]) then begin
...
begin
  ...
  if(Allowed) then begin
    if Assigned(m_Mediator.Grid.FOnFilterChar)
      then m_Mediator.Grid.FOnFilterChar(Self, m_nCellX, m_nCellY, Key, Allowed);
  end;
...
//end;
//
//procedure TNiceGridSync.SetGrid(const Value: TBrvGrid);
//begin
//  if (FGrid <> Value) then
//  begin
//    FGrid := Value;
...
constructor TBrvInplace.Create(AGrid: TBrvGrid);
{-----------------------------------------------------------------------------
  Procedure: Create
  Author:    nbi
  Date:      15-Oct-2014
  Arguments: AGrid: TBrvGrid
  Result:    None
-----------------------------------------------------------------------------}
begin
  m_Grid := AGrid;
...
function TBrvInplace.GetHandle: THandle;
{-----------------------------------------------------------------------------
  Procedure: GetHandle
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplace.HideEdit;
{-----------------------------------------------------------------------------
  Procedure: HideEdit
  Author:    nbi
  Date:      15-Oct-2014
...
-----------------------------------------------------------------------------}
begin
  (m_Control as IBrvGridInplaceEditor).HideEdit;
end;



procedure TBrvInplace.ShowEdit(X, Y: Integer);
{-----------------------------------------------------------------------------
  Procedure: ShowEdit
  Author:    nbi
  Date:      15-Oct-2014
...
begin
  ...
  case et of
    brvgetCombo, brvgetComboNoEditText:
      m_Control := TBrvInplaceCombo.Create(Self);
    brvgetEdit, brvgetEditInteger, brvgetEditFloat:
      m_Control := TBrvInplaceEdit.Create(Self);
  end;

  (m_Control as IBrvGridInplaceEditor).ShowEdit(X, Y);
end;
...
procedure TBrvInplaceCombo.Change;
{-----------------------------------------------------------------------------
  Procedure: Change
  Author:    nbi
  Date:      15-Oct-2014
...
begin
  inherited;
  m_Mediator.Grid.InternalSetCell(m_CellX, m_CellY, Text, True);
end;




procedure TBrvInplaceCombo.CMWantSpecialKey(var Message: TWMKey);
{-----------------------------------------------------------------------------
  Procedure: CMWantSpecialKey
  Author:    nbi
  Date:      17-Oct-2014
...
constructor TBrvInplaceCombo.Create(AMediator: TBrvInplace);
{-----------------------------------------------------------------------------
  Procedure: Create
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplaceCombo.HideEdit;
{-----------------------------------------------------------------------------
  Procedure: HideEdit
  Author:    nbi
  Date:      15-Oct-2014
...
procedure TBrvInplaceCombo.KeyPress(var Key: Char);
{-----------------------------------------------------------------------------
  Procedure: KeyPress
  Author:    nbi
  Date:      16-Oct-2014
...
procedure TBrvInplaceCombo.ShowEdit(X, Y: Integer);
{-----------------------------------------------------------------------------
  Procedure: ShowEdit
  Author:    nbi
  Date:      15-Oct-2014
...
begin
  ...
  et := Grid.Columns[X].EditorType;
  if(et = brvgetComboNoEditText) then
...
//    then Rc.Right := Rc.Right + 1;
  ...
  if(brvgoExcel in Mediator.Grid.Options) then
    InflateRect(Rc, -4, -3);
...
procedure TBrvInplaceCombo.WMKillFocus(var Msg: TWMKillFocus);
{-----------------------------------------------------------------------------
  Procedure: WMKillFocus
  Author:    nbi
  Date:      31-Oct-2014
...
procedure TBrvInplaceCombo.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
end;

{ TBrvGridRowsByIndexEnumerator }

constructor TBrvGridRowsByIndexEnumerator.Create(ANiceGrid: TBrvGrid);
{-----------------------------------------------------------------------------
  Procedure: Create
  Author:    nbi
  Date:      16-Oct-2014
  Arguments: ANiceGrid: TBrvGrid
  Result:    None
-----------------------------------------------------------------------------}
begin
  m_nRowIndex := -1;
...
function TBrvGridRowsByIndexEnumerator.GetCurrent: Integer;
{-----------------------------------------------------------------------------
...
function TBrvGridRowsByIndexEnumerator.GetEnumerator: TBrvGridRowsByIndexEnumerator;
{-----------------------------------------------------------------------------
...
function TBrvGridRowsByIndexEnumerator.MoveNext: Boolean;
{-----------------------------------------------------------------------------
  Procedure: MoveNext
  Author:    nbi
  Date:      16-Oct-2014
